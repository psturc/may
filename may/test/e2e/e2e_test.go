...
var _ = Describe("Manager", Ordered, func() {
	...
	BeforeAll(func() {
		...
		cmd := exec.Command("kubectl", "create", "ns", namespace, "--dry-run=client", "--output=yaml")
		ns, err := utils.Run(cmd)
		Expect(err).NotTo(HaveOccurred(), "Failed to generate namespace's yaml specification")
		cmd = exec.Command("kubectl", "apply", "-f", "-", "--server-side")
		cmd.Stdin = strings.NewReader(ns)
		_, err = utils.Run(cmd)
		Expect(err).NotTo(HaveOccurred(), "Failed to apply namespace's yaml specification")
	...
	AfterAll(func() {
		if os.Getenv("SKIP_E2E_CLEANUP") == "true" {
			_, _ = fmt.Fprintf(GinkgoWriter, "SKIP_E2E_CLEANUP is set, skipping cleanup\n")
			return
		}
	...
	Context("Manager", func() {
		...
		It("should ensure the metrics endpoint is serving metrics", Label("smoke"), func() {
			...
			cmd := exec.Command("kubectl", "create", "clusterrolebinding", metricsRoleBindingName,
				...
				"-o",
				"yaml",
				"--clusterrole=may-metrics-reader",
				"--dry-run=client",
				"--output=yaml",
